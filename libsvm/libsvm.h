 ...
 * Author: Jan Hendriks
 *
 * Created on 6. Mai 2011, 15:00
 * Wrapper interface for libsvm
...
public:
    ...
    void freeMem() {
        ...
        svm_free_and_destroy_model(&this->model); // The model is used in training and prediction specific steps
    }
...
//                printf("Value: '%f'\n", x_space[j].value);
    ...
    void saveModelToFile(const std::string _modelFileName, const std::string _identifier = "libsvm") {
        if (svm_save_model(_modelFileName.c_str(), this->getInstance()->getModel())) {
    ...
    void loadModelFromFile(const std::string _modelFileName, const std::string _identifier = "libsvm") {
        ...
        if (this->predictionDataStructsUsed && this->model != NULL) {
//            svm_free_and_destroy_model(&this->model);
            delete this->model; // clear the previous model and deallocate/free the occupied memory
            this->model = NULL; // set the pointer to NULL
        }
        printf("Loading model from file '%s'\n", _modelFileName.c_str());
        this->model = svm_load_model(_modelFileName.c_str());
        if (this->model == NULL) {
            printf("Loading of SVM model failed!\n");
            exit(EXIT_FAILURE);
        }

        int classNr = svm_get_nr_class(this->model);
        int labels[classNr];
        for (int i = 0; i < classNr; ++i) {
                labels[i] = 0;
        }
        svm_get_labels(this->model, labels);
        double b = -(this->model->rho[0]);
        double probA = (this->model->probA[0]);
        double probB = (this->model->probB[0]);
        
        int kernelType = this->model->param.kernel_type;
        printf("Loaded model: SVM type %d, Kernel type %d, %d classes: labels %d, %d, #SVs %d, bias b %3.5f, probA %3.5f, probB %3.5f\n", svm_get_svm_type(this->model), kernelType, classNr, labels[0], labels[1], this->model->l, b, probA, probB);
...
/*
    float predictLabel(std::vector<float>* _sample, double* prob_estimates) {
        if (this->model == NULL) { /// @WARNING This is unelegant, valgrind complains because of check against uninitialized variable
...
//        printf("SVM type: %u, # classes: %u\n", svm_get_svm_type(this->model), svm_get_nr_class(this->model));
//        this->predictLabel(x);
//        float predict_label = svm_predict(this->model, x);
        float predict_label = svm_predict_probability(this->model, x, prob_estimates);
...
//printf("Predict function before pred 2nd svm_node value: %3.6f\n", _sample[1].value);
        float predict_label = svm_predict_probability(this->model, _sample, _probEstimate);
...
//        printf("Model kernel type %d\n", this->model->param.kernel_type);
        return predict_label;
//        return svm_predict(this->model, _sample);
    }
...
//        this->model = svm_train(&prob, &param);
    ...
    void getSingleDetectingVector(std::vector<prec>& singleDetectorVector, std::vector<unsigned int>& singleDetectorVectorIndices) {
        ...
        double b = -(this->model->rho[0]); // This is the b value from the SVM, assumes that first the positive labels are read in (otherwise, use double b = (*_model->rho); )
        printf("b: %+3.5f\n", b);
        // Walk over every support vector and build a single vector
        for (unsigned long ssv = 0; ssv < this->getModel()->l; ++ssv) { // Walks over available classes (e.g. +1, -1 representing positive and negative training samples)
//                        printf("Support vector #%lu \n", ssv);
            // Retrive the current support vector from the training set
            svm_node* singleSupportVector = this->getModel()->SV[ssv]; // Get next support vector ssv==class, 2nd index is the component of the SV
            //            _prob->x[singleSupportVector->index];
            // sv_coef[i] = alpha[i]*sign(label[i]) = alpha[i] * y[i], where i is the training instance, y[i] in [+1,-1]
            double alpha = this->getModel()->sv_coef[0][ssv];
